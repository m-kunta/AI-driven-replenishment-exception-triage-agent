...
scripts/                       # CLI interaction scripts (run_triage, run_backtest, run_daily)
...
- **Phase 13 — Agentic Engagement:** In progress. The execution slice is complete with action modal submission, inline action history/status, a global paginated/filterable Action History page (`/actions`), FastAPI action endpoints, retry support, a real Slack webhook adapter (`SlackWebhookAdapter`, used automatically when `SLACK_WEBHOOK_URL` is set), planner-only gating for `STORE_CHECK` / `VENDOR_FOLLOW_UP` and for override approve/reject, per-user credentials (`API_USERS`) with per-user role resolution, an in-memory pipeline run registry with live status polling (`GET /pipeline/status/{run_id}`), an override-analytics endpoint (`GET /overrides/stats`), a cron-friendly daily-run script (`scripts/run_daily.py`), and a Settings page backed by runtime `/settings` and `/models` inspection endpoints. Broader ERP-specific adapters and fuller RBAC remain future work.
